...
The ERP dashboard's "premium" bar is **disciplined reuse of the existing glass/solid token system** (`app/globals.css`) + `components/jce/*` primitives — NEVER the marketing electrified/dark kit, raw hex/oklch, bracketed `var()`, or new color tokens. Worked examples: **BDD Sales Orders (B1/B2)** (register + edit-with-audit record w/ `Meter`) and **BDD Offers (B3/B4)** (register + immutable event-stream record — derived `MetricCard` strip, no `Meter`).
