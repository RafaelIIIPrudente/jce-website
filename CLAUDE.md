...
- **Responsive / mobile-first.** Every dashboard screen must read well from ~320px up — design mobile-first, layer `sm:`/`lg:` up ([Tailwind responsive](https://tailwindcss.com/docs/responsive-design)). KPI/metric grids stack then widen (`grid-cols-1 sm:grid-cols-2 lg:grid-cols-4`; counts may stay 2-up); **long money** uses `pesoCompact()` on summary tiles and stacks to one column for precise figures (never clip a peso) — pair with responsive type (`text-ui-22 sm:text-ui-28` heroes). Dense registers (`LedgerGrid`) scroll horizontally inside their solid surface; **wide comparison matrices** (`ComparisonMatrix`) scroll horizontally with the criterion (row-header) column **sticky** (`sticky left-0`, opaque `--table-head` bg) so the row labels stay in view while panning supplier columns and the page never overflows (B6); **append-only audit logs** (`AuditLog`) scroll horizontally with the leading **Timestamp column sticky** (opt-in `jtable-sticky-first` — opaque per-zebra-parity token bg, hairline frozen-edge shadow) and swap a wide multi-option area `Segmented` to a **full-width `Select` below `lg`** so the 9-option filter stays tappable and never overflows (B11); **master-detail / inbox layouts** (`Inquiries` B10) collapse from a two-pane `lg:grid-cols-[360px_1fr]` split to a **single-pane list⇄detail flow** below `lg` — selecting a lead swaps to the detail with a `← Inbox` back affordance, the list/detail toggled via `hidden lg:block` / `hidden lg:flex` (no layout shift, reduced-motion-safe since it's a display swap, not an animation); primary actions go full-width on phones (`w-full sm:w-auto`); dialogs are scrollable + viewport-bounded (`max-h-[90dvh] overflow-y-auto`, fields `grid-cols-1 sm:grid-cols-2`). **Sheet/drawer editors** are full-width + scrollable (`w-full sm:max-w-xl`, sticky header/footer with a `flex-1 overflow-y-auto` body) and collapse their field pairs to one column on mobile (`grid-cols-1 sm:grid-cols-2`); **tab bars** go full-width / horizontally scrollable with ≥44px triggers (`min-h-11`); **card-row lists** stack identity above controls then go inline at `lg`, with no fixed-width control that clips at ~360px (e.g. a Status select is `w-full sm:w-40`). No horizontal page scroll, no clipped controls. **Large screens / ultrawide:** the canonical wide-surface cap is **`max-w-app`** (`--container-app`, 110rem/1760px) — lists, dashboards & data-workspaces (and their module sub-nav wrappers, for edge alignment) grow to it then center; **record/detail pages, create/edit & document-issue forms, and `ch`/`prose` reading content stay at their narrower reading widths** (don't widen them — wide forms & long lines hurt readability). Use the extra room with a **`2xl:` density tier only where item counts justify more columns** (a dynamic card grid `lg:grid-cols-3 2xl:grid-cols-4`; a 6-item strip `lg:grid-cols-3 2xl:grid-cols-6`; a master-detail split `lg:grid-cols-[360px_1fr] 2xl:grid-cols-[420px_1fr]`) — a fixed 4-KPI strip (`grid-cols-2 lg:grid-cols-4`) stays 4-up, tiles just widen; the shell adds `2xl:p-8` inset.
- **App shell / chrome** (`AppShell` → `AppSidebar` + `AppTopbar`, the glass wrapper around EVERY dashboard module). Premium `.glass-nav` rail + top bar over `.jce-backdrop`. The rail is a **desktop-only persistent, collapsible** rail (`hidden lg:flex`, 256↔72px via `transition-[width]`) whose collapse control is a **subtle right-aligned edge affordance** (compact chevron + light label, `min-h-11` — never a full-width block) that becomes a centered icon when collapsed. Below `lg` the rail is hidden and the SAME RBAC nav (`visibleModules`/`getGrant` "R" tags, shared `SidebarNav`) renders in an **off-canvas `Sheet` drawer** (`side="left"`, scrim + focus-trap + Escape built in, closes on nav-select), opened by a `lg:hidden` **hamburger** in the top bar (`aria-controls`/`aria-expanded`); the top bar also swaps desktop breadcrumbs for a **compact current-page label** below `sm`. A separate `mobileOpen` state drives the drawer (never overload the desktop `collapsed`). Chrome controls use token radii (`--r-input`/`--r-chip`), `bg-card`/`bg-card/70` (never `bg-white/70`), the `text-ui-*` scale (no raw `text-[Npx]`), ≥44px targets, `focus-ring-jce`; the collapse-width + drawer-slide both fall back to no-motion under `prefers-reduced-motion` (global reduce block). `h-[100dvh]` so mobile browser chrome doesn't clip the shell.
