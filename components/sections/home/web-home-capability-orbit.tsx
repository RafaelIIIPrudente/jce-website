...
function OrbitView({
  ...
  const paused = activeId !== null || hintId !== null || !onScreen;
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className="absolute inset-[7%] rounded-full border border-jce-dark-line"
      />
      ...
      <div className="absolute top-1/2 left-1/2 grid size-28 -translate-x-1/2 -translate-y-1/2 place-items-center sm:size-32">
        ...
        <OmegaMark
          pulse
          strokeWidth={5}
          className="size-14 text-jce-green-500 sm:size-16"
        />
      </div>
        ...
        {NODES.map((node, i) => {
          ...
          return (
            <button
              ...
              onMouseEnter={() => setHintId(node.id)}
              onMouseLeave={() =>
                setHintId((cur) => (cur === node.id ? null : cur))
              }
              onFocus={() => setHintId(node.id)}
              onBlur={() => setHintId((cur) => (cur === node.id ? null : cur))}
              aria-expanded={isActive}
              aria-controls={panelId}
              aria-label={`${node.title} — ${isActive ? "hide" : "show"} details`}
              style={{ left: `${x}%`, top: `${y}%` }}
              className={cn(
                "focus-ring-cyan absolute grid size-14 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border backdrop-blur-sm transition-colors",
                isActive
      ...
      {/* Expanded card — dark token surface centred over the core. */}
      {active ? (
        <>
          <button
            type="button"
      ...
      {NODES.map((node) => {
        ...
        return (
          ...
          >
            ...
            >
              <span className="grid size-11 shrink-0 place-items-center rounded-full border border-jce-dark-line bg-jce-dark/50 text-jce-cyan-bright">
