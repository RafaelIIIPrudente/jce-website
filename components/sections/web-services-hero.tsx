...
// S3 Services hero — the electrified dark idiom at inner-page scale (same language
// as web-about-hero): an ambient circuit field + a large faint Ω brand watermark +
// a rising CurrentTrace that draws on view. Carries the page's single <h1>.
// Deliberately photo-less — keeps the hero brand-led and avoids the next/image
// priority-LCP src-collision (no priority image exists on the page). Mobile-first:
// the headline clamps from 360px up; the decorative layers are clipped, behind
// content, and freeze to a static render under prefers-reduced-motion.
  ...
  return (
    <section className="dark-section circuit-field relative isolate overflow-hidden">
      ...
      <div className="mx-auto w-full max-w-6xl px-5 py-24 sm:py-28 md:py-32">
        <p className="kicker text-jce-cyan-bright">Capabilities</p>
        <h1 className="mt-4 max-w-[20ch] text-[clamp(30px,6vw,60px)] leading-[1.04] font-bold tracking-[-0.02em] text-balance text-jce-dark-ink">
          Full-scope power{" "}
          <span className="text-jce-cyan-bright">engineering services.</span>
        </h1>
        <p className="mt-5 max-w-[60ch] text-ui-16 text-pretty text-jce-dark-ink-2 sm:text-ui-18">
          Turnkey EPC and specialized power services from distribution voltages
          up to 230 KV — one accountable team from study to handover.
        </p>
      </div>
    </section>
  );
