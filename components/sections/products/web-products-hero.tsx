...
// S5 Products hero — the electrified dark idiom at inner-page scale (same language
// as web-about-hero / web-services-hero): an ambient circuit field + a large faint
// Ω brand watermark + a rising CurrentTrace that draws on view. Carries the page's
// single <h1>. Deliberately photo-less — keeps the hero brand-led and sidesteps the
// next/image priority-LCP src-collision (no priority image exists on this page).
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto w-full max-w-6xl px-5 py-24 sm:py-28 md:py-32">
        <p className="kicker text-jce-cyan-bright">What we supply</p>
        <h1 className="mt-4 max-w-[20ch] text-[clamp(30px,6vw,60px)] leading-[1.04] font-bold tracking-[-0.02em] text-balance text-jce-dark-ink">
          Power equipment{" "}
          <span className="text-jce-cyan-bright">we supply.</span>
        </h1>
        <p className="mt-5 max-w-[60ch] text-ui-16 text-pretty text-jce-dark-ink-2 sm:text-ui-18">
          Specified, supplied and integrated by JCE — kept distinct from our EPC
          services. Exclusive Philippine distributor of Shenda Electric.
        </p>
      </div>
    </section>
  );
