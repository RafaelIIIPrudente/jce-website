...
// S7 Careers hero — the electrified dark idiom at inner-page scale, the same
// language as web-about-hero / web-services-hero / web-products-hero: an ambient
// circuit field, a large faint Ω brand watermark, and a rising CurrentTrace that
// draws on view (anime.js, client-only, frozen under reduced-motion). Carries the
// page's single <h1>. Deliberately photo-less — brand-led, and avoids the
  ...
  return (
    <section className="dark-section circuit-field relative isolate overflow-hidden">
      ...
      <div className="mx-auto w-full max-w-6xl px-5 py-24 sm:py-28 md:py-32">
        <p className="kicker text-jce-cyan-bright">Join us</p>
        <h1 className="mt-4 max-w-[20ch] text-[clamp(30px,6vw,60px)] leading-[1.04] font-bold tracking-[-0.02em] text-balance text-jce-dark-ink">
          Build what{" "}
          <span className="text-jce-cyan-bright">powers the Philippines.</span>
        </h1>
        <p className="mt-5 max-w-[60ch] text-ui-16 text-pretty text-jce-dark-ink-2 sm:text-ui-18">
          From substations to the national grid — open roles across engineering,
          project management, construction and purchasing.
        </p>
      </div>
    </section>
  );
